...
function                                            genPathClass($classname)
{
        ...
        $dirName = (in_array($aClass[0], array('Request', 'Webservices'))) ? $aClass[0] : 'Objects';
        $objectName = end($aClass);
        return (__ROOTDIR__ . "/$dirName/$objectName/$objectName.php");
}
...
function                                            initConfig()
{
                ...
                (($dataIniFile = parse_ini_file($iniConf['config'], true))))
        {
                ...
                if (array_key_exists('dev', $dataIniFile))
                {
                        \Webservices\Mail::pushToInDevArray($dataIniFile['dev']['email']);
                }
                \Webservices\Request::loadKeyAPI((array_key_exists('keys', $dataIniFile)) ? $dataIniFile['keys'] : false);
        }
        ...
        if ((pathinfo($iniConf['database'], PATHINFO_EXTENSION) == 'ini') && (file_exists($iniConf['database'])) &&
         ...
         \Webservices\Mail::setEnv(__ENV__);
}
